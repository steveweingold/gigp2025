forward
global type u_tabpg_depm_items from u_tabpg_appinfo_base
end type
end forward

global type u_tabpg_depm_items from u_tabpg_appinfo_base
string tag = "DEPM Items"
integer height = 2000
string text = "DEPM"
string picturename = "PictureListBox!"
end type
global u_tabpg_depm_items u_tabpg_depm_items

type variables
boolean ib_updateSEQR = False
end variables

on u_tabpg_depm_items.create
call super::create
end on

on u_tabpg_depm_items.destroy
call super::destroy
end on
...
type dw_1 from u_tabpg_appinfo_base`dw_1 within u_tabpg_depm_items
integer x = 18
integer y = 12
integer width = 2821
integer height = 1968
string dataobject = "d_proj_project_depm_dates"
end type
...
string ls_status, ls_null, ls_ind, ls_code, ls_user
long  ll_count
...
Choose Case dwo.name
	...
	Case  'keydate_value'
		...
		If ls_code = 'readyFINPLANSSUB' Then
			select count(*)
			into :ll_count
			from gigp_key_dates
			where gigp_id = :gl_gigp_id
			and ref_code = 'readyPLANSSPECS';
			...
			Else
				...
				values (:gl_gigp_id, 'readyPLANSSPECS', :ldt_date, 'Auto-updated from DEPM dates.', :ls_user, getdate());
